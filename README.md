...
You can suppress auto blinking defined in the `VRC Avatar Descriptor` whenever you want, while keeping the Eye Look feature working.  
...
2. Configure your animations so that the `Suppress Blink` property is set to `True` only when blinking should be suppressed, and `False` otherwise.
